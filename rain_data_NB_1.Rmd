...
```{r summary statistics,include=FALSE}
  ...
  rain_stats_function_1 <- function(files) {
      ...
      # create empty dataframe, on first pass; populate the df with results
      if(k == 1) {
        stats_df<-data.frame(NULL)
        stats_df <- stats_summary
      }
      
      # iteratively cbind the results into a dataframe
      ...
      colnames(stats_df) <- c("Statistic","Hourly","Daily", "Monthly", "Yearly")
      
      # cbind the 'stats_summary_5_min' results into the dataframe
      stats_df <- cbind(stats_df,"5 Minute" = stats_summary_5_min)
    ...
    CSV_outfile<-file.path(stats_folder_1, CSV_file_name)
     ...
     # write the results to .CSV file
    write.table(stats_df,CSV_outfile,sep=",",col.names=TRUE,row.names=TRUE)
