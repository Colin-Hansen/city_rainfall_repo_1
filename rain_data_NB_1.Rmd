...
```{r summary statistics,echo= FALSE,include=FALSE}
  ...
  rain_stats_function_1 <- function(files) {
      ...
      table_caption<-paste0(site_prefix,' Summmary: 5 Minute Data')
    
      # read the saved .CSV file
      # stats_table<-read.table('data_out/S01/CSV/S01.csv',sep=",",header=TRUE,stringsAsFactors = FALSE)
    
      
      stats_5_min %>% knitr::kable(caption = table_caption, digits = 2) %>% 
      ...
      stats_df[1,1:5] <- c("Hourly","Daily","Weekly","Monthly","Yearly")
...
```{r, extract events MIT_1,echo=FALSE,}
...
```{r, kable testing,results='asis'}
  ...
  # stats_table %>% knitr::kable(caption = table_caption, digits = 2) %>% 
    kableExtra::kable_styling(bootstrap_options = "striped")
