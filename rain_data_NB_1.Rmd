...
```{r summary statistics,include=FALSE}
  ...
  rain_stats_function_1 <- function(files) {
      ...
      # get the 5- minute rainfall - WITHOUT ZEROES
      
      rain_5_min_NZ <- rain_5_min %>% filter(Rainfall > 0) %>% ungroup() %>% select(Rainfall)
     
      # get the hourly rainfall - WITHOUT ZEROES
      
      rain_hourly_NZ <- rain_hourly %>% filter(Rainfall > 0) %>% ungroup() %>% select(Rainfall)
      
      # get the daily rainfall - WITHOUT ZEROES
      
      rain_daily_NZ <- rain_daily %>% filter(Rainfall > 0) %>% ungroup() %>% select(Rainfall)
      
      # get the monthly rainfall - WITHOUT ZEROES
      
      rain_monthly_NZ <- rain_monthly %>% filter(Rainfall > 0) %>% ungroup() %>% select(Rainfall)
      
      # get the yearly rainfall - WITHOUT ZEROES
      
      rain_yearly_NZ <- rain_yearly %>% filter(Rainfall > 0) %>% ungroup() %>% select(Rainfall)
      
      
      ## STOPPED HERE 9-JAN-2021
