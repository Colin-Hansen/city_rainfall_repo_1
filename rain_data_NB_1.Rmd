...
if(dir.exists('data_out') == FALSE) dir.create('data_out')
  ...
  # combine data and time and convert to POSIXlt class
  
  Date_chr <- as.character(rainfall_data$Date)
  hms_chr <- as.character(rainfall_data$Hour.Minute.Seconds)
  
  temp_date_time<-paste(Date_chr,hms_chr)
  # dformat<-"%Y/%m/%d %H:%M"
   dformat<-"%Y-%m-%d %H:%M"
  Date.Time<-strptime(temp_date_time,dformat)
  ...
  # add date-time from individual date and time components
