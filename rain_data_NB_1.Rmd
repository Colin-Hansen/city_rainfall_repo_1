...
```{r summary statistics,echo= FALSE,include=FALSE}
  ...
  rain_stats_function_1 <- function(files) {
      ...
      # print(dim(stats_5_min))
      
      colnames(stats_5_min) <- c("Value")
      ...
      write.table(stats_5_min,CSV_outfile,sep=",",col.names=TRUE,row.names=TRUE)
      ...
      # get the 5 minute rainfall - WITHOUT RAINFALL EQUAL TO ZERO
      rain_5_min_NZ <- rain_5_min %>% filter(Rainfall > 0) %>% mutate(summary.rainfall = Rainfall) %>% ungroup() %>% select(summary.rainfall)
      
     
      
      
      # get the hourly rainfall - WITHOUT RAINFALL EQUAL TO ZERO
      ...
      rain_hourly_NZ <- rain_5_min %>% filter(Rainfall > 0) %>% group_by(Year,Month,Day,Hour) %>% summarize(summary.rainfall=sum(Rainfall)) %>% ungroup() %>% select(summary.rainfall)
      
      # get the daily rainfall - WITHOUT RAINFALL EQUAL TO ZERO
      rain_daily_NZ <- rain_5_min %>% filter(Rainfall > 0) %>% group_by(Year,Month,Day) %>% summarize(summary.rainfall=sum(Rainfall)) %>% ungroup() %>% select(summary.rainfall)
      
       # get the monthly rainfall - WITHOUT RAINFALL EQUAL TO ZERO
      rain_monthly_NZ <- rain_5_min %>% filter(Rainfall > 0) %>% group_by(Year,Month) %>% summarize(summary.rainfall=sum(Rainfall)) %>% ungroup() %>% select(summary.rainfall)
      
       # get the yearly rainfall - WITHOUT RAINFALL EQUAL TO ZERO
      rain_yearly_NZ <- rain_5_min %>% filter(Rainfall > 0) %>% group_by(Year) %>% summarize(summary.rainfall=sum(Rainfall)) %>% ungroup() %>% select(summary.rainfall)
      
      
      # create a list of the rain interval results
      
      rain_list_1 <- list(rain_5_min_NZ,rain_hourly_NZ,rain_daily_NZ,rain_monthly_NZ,rain_yearly_NZ)
      ...
      stats_df[1,1:6] <- c("5 Minute","5 Minute.Non.Zero","Hourly.Non.Zero","Daily.Non.Zero","Monthly.Non.Zero","Yearly.Non.Zero")
