...
```{r summary statistics, echo = FALSE}
  ...
  # seq along the .CSV files; generate basic summary stastics
    ...
    # print(csv_names)
     ...
     for (i in seq_along(csv_names)) {
      ...
      # create directory to save statistics results - hourly / daily / weekly / monthly / yearly data
      ...
      # add a new field 'Site.Name'
      
      Site.Name <- rep(site_prefix,nrow(stats_5_min))
      
      stats_5_min<-cbind(Site.Name,stats_5_min)
     
      colnames(stats_5_min) <- c("Site.Name","Value")
      
      # create a file name to save the 5 minute stats
      CSV_file_name<-paste0(site_prefix,"_stats_summary_5_min.csv")
      CSV_outfile<-file.path(stats_folder_1, CSV_file_name)
        
      # write the results to .CSV file; ** Note: col.names = NA to ensure the column name is written over the data, not the rownames
      
      write.table(stats_5_min,CSV_outfile,sep=",",col.names=NA,row.names=TRUE)
     
      # combine the Site.Name and the total data count
      
      temp_count_df <- data.frame("Site.Name" = site_prefix,"Data.Count" = n.total)
      
      print(temp_count_df)
      
      # # assign 'stats_5_min' data count result to the dataframe, only on the first pass
      if(i == 1) {
         
      # # create empty dataframe to store the results
      count_df<-data.frame(NULL)
      ...
      if(i > 1) {
        count_df <- rbind(count_df,temp_count_df)
      }
      
      
      
      # # extract out the Site.Name and 'n.total'
      # data_count <- stats_5_min[4,]
      # 
      # # convert to df; clean up the col names
      # data_count <- as.data.frame(data_count)
      # 
      # # colnames(data_count) <- c("Site.Name","Data.Count")
      # colnames(data_count) <- c("Data.Count")
      # 
      # # print(data_count)
      # 
      # # assign 'stats_5_min' data count result to the dataframe, only on the first pass
      ...
      # # iteratively rbind the results into a dataframe, starting at the second iteration
      # if(i > 1) {
      #   count_df <- rbind(count_df,data_count)
      # }
      ...
      rain_weekly <- rain_5_min %>% group_by(Year,Month,Week) %>% summarize(summary.rainfall=sum(Rainfall)) %>% ungroup() %>% select(summary.rainfall)
      ...
      # rain_list_1 <- list(rain_hourly_NZ,rain_daily_NZ,rain_weekly_NZ,rain_monthly_NZ,rain_yearly_NZ)
      ...
      # print(attributes(rain_list_1[[k]]))
   
      # use smry from 'hydroTSM' package
     
      # note - double square brackets to access individual element of the list
      stats_summary_hydroTSM <- hydroTSM:::smry(rain_list_1[[k]]$summary.rainfall,na.rm = TRUE)
      
      # print(stats_summary_hydroTSM)
      
       # assign stats result to the dataframe, only on the first pass
      if(k == 1) {
        stats_df <- stats_summary_hydroTSM
      }
      
      # iteratively cbind the results into a dataframe, starting at the second iteration
      if(k > 1) {
        stats_df <- cbind(stats_df,stats_summary_hydroTSM)
      }
      ...
      # add column names
      colnames(stats_df) <- c("Hourly","Daily","Weekly","Monthly","Yearly")
      
      
      # add Site.Name to dataframe
      
      Site.Name <- rep(site_prefix,nrow(stats_df))
      
      stats_df<- data.frame(Site.Name,stats_df)
      ...
      CSV_outfile<-file.path(stats_folder_2, CSV_file_name)
       ...
       # write the results to .CSV file
       write.table(stats_df,CSV_outfile,sep=",",col.names=NA,row.names=TRUE)
  ...
  # *************************************************************
      ...
      # with 'count_df': save .csv file and plot bar chart of data count all sites
     
      # create a directory to save the compiled results
      dir_3<-paste0('data_out',"/Stats_Summary_3")
      
      # create (if necessary) 'data_out/MASTER_CSV_Annual_Results' directory
      if(dir.exists(dir_3) == FALSE) dir.create(dir_3)
      
      # create file path and name to save results
      temp_name<-paste0(dir_3,"/total_count_5_min_raw_all_sites.csv")
      
      # write the above results to .CSV files
      write.table(count_df,temp_name,sep=",",col.names=TRUE,row.names=FALSE)
      ...
      outfile_name<-'total_5_minute_data_by_site.png'
      
      # specify directory name
      PNG_folder_name<-dir_3
      
    p1<-ggplot(data=count_df, aes(x = Site.Name,y = Data.Count,fill=Site.Name)) +
  ...
  table_caption<-paste0('SXX',': Statistics Summmary')

  # read the saved .CSV file
  stats_table<-read.table('data_out/Stats_Summary_1/S01_stats_summary_5_min.csv',sep=",",header=TRUE,stringsAsFactors = FALSE)

  
  stats_table %>% knitr::kable(caption = table_caption, digits = 2) %>% kableExtra::kable_styling(bootstrap_options = "striped")
...
```{r kable testing 1, eval = FALSE, results='asis'}
