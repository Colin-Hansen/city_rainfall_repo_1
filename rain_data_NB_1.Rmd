...
```{r summary statistics,include=FALSE}
  ...
  rain_stats_function_1 <- function(files) {
      ...
      if(k == 1) {
        new_stats_df<-data.frame(NULL)
        new_stats_df <- stats_summary
      }
      
      # iteratively cbind the results into a dataframe
      new_stats_df <- cbind(new_stats_df,stats_summary)
    ...
    CSV_outfile<-file.path(stats_folder_1, CSV_file_name)
     ...
     # write the results to .CSV file
    write.table(new_stats_df,CSV_outfile,sep=",",col.names=TRUE,row.names=TRUE)
