...
```{r summary statistics,include=FALSE}
  ...
  rain_stats_function_1 <- function(files) {
      ...
      ## STOPPED HERE 01-JAN-2021
      
      stats_5_min <- rainfall_data %>% summarize(mean=mean(Rainfall),
                                                 sd=sd(Rainfall),
                                                 min=min(Rainfall),
      ...
      # transpose the results before writing .CSV file
      stats_5_min <- t(stats_5_min )
      
      # convert from matrix to dataframe; add column names
      stats_5_min <- as.data.frame(stats_5_min)
      
     # function to return basic statistics; Note: just pass the Rainfall data to the function
      
      basic_stats_function <- function(df) {
        
        summarize(mean=mean(df),
                  sd=sd(df),
                  min=min(df),
                  Q1=quantile(df,probs=0.25),
                  median=median(df),
                  Q3=quantile(df,probs=0.75),
                  max=max(df),
                  CV_pct=(sd/mean)*100,
                  total.count=n(),
                  n.valid=sum(!is.na(df)),
                  pct.valid=(n.valid/total.count)*100)
      ...
      TEST_stats_function <- function(df) {
     ...
     # get the houry rainfall; Note: you have to add 'ungroup()' before the select command to extract only the hourly.rainfall *** 
        
      hourly_rainfall <- rainfall_data %>% group_by(Year,Month,Day,Hour) %>% summarize(hourly.rainfall=sum(Rainfall)) %>% ungroup() %>% select(hourly.rainfall)
      
      
      # print(class(hourly_rainfall))
      
      # print(hourly_rainfall)
      # 
      # print(str(hourly_rainfall))
      
      # test_hourly_stats <- basic_stats_function(hourly_rainfall)
      ...
      test_mean <- TEST_stats_function(dummy_data)
      
      sprintf("the mean is %f",test_mean)
      
      
     
      # write the results to .CSV file
      # write.csv(stats_5_min,CSV_outfile,sep=",",col.names=TRUE,row.names = FALSE)
       write.csv(stats_5_min,CSV_outfile,sep=",",col.names=TRUE,row.names = TRUE)
       
      # write.csv(test_hourly_stats,CSV_outfile,sep=",",col.names=TRUE,row.names = TRUE)
