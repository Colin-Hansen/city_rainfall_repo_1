...
```{r charts-part-3a,echo = FALSE,message=FALSE, warning=FALSE}
    ...
    # create scatterplot of City versus YYC yearly rainfall data
      ...
      p5<-ggplot() +
      geom_point(data = join_City_and_YYC,aes(x = Rainfall.mm.YYC,y = Rainfall.mm.City.gauge,colour="red")) +
      geom_abline(intercept = 0,slope = 1,aes(linetype = "dashed")) +
        theme(axis.title.x = element_text(size = 11),axis.text.x = element_text(hjust=1,size=11)) +
        theme(axis.title.y = element_text(size = 11),axis.text.y = element_text(hjust=1,size=11)) +
        # note: data outside 0 to 500 will not be plotted
        ...
        ggtitle(chart_title)
      # outfile<-file.path(PNG_folder_name,outfile_name)
      # ggsave(outfile,p5,width= 6, height=3.708, dpi=600,units=c("in"))
      print(p5)
      
      #try to create two plots and add them together
