...
# the read function was removed becuase the 5-minute rainfall data was already read in a previous chunk
    ...
    for (i in seq_along(MIT_value)) {
      ...
      # last_line_df<- RainEvents_df_long %>% group_by(.id) %>% summarize(depth=last(cumulative.rainfall), duration.minutes=last(event.duration.minutes),duration.hours=last(event.duration.hours),rainrate.mm.hr=last(event.rainrate.mm.hr),max.intensity.mm.hr=max(intensity.mm.hr,na.rm=TRUE))
       ...
       depth<- RainEvents_df_long %>% group_by(.id) %>% summarize(depth=last(cumulative.rainfall)) %>% select(depth) 
       
       duration_hours<- RainEvents_df_long %>% group_by(.id) %>% summarize(duration=last(event.duration.hours)) %>% select(duration)
       
       intensity<- RainEvents_df_long %>% group_by(.id) %>% summarize(intensity=last(event.rainrate.mm.hr)) %>% select(intensity)
             ...
             for (m in seq_along(x)) {
               ...
               # print(x$Date.Time[[m]])
                ...
                if (m > 1) {
                  ...
                  gap_start <- max(x$Date.Time[[m-1]])
                  
                  gap_end <- min(x$Date.Time[[m]])
     ...
     # use 'map' to apply 'gap_times_function' to the list of dataframes
      ...
      test_gap_results <- RainEvents_df_long_split %>% map(summary)
      
      ## STOPPED HERE 20-FEB-2021; gap_results list is empty of results 
