...
# cat("\\newpage")
  ...
  # read a single .CSV file; generate basic summary stastics
    ...
    csv_name<-list.files('data_out/MASTER_CSV_5_min')
      ...
      # explanation: 'plyr' has to be detached in order for 'dplyr' summarize to work properly. 
      # ... but if I just run this chunk, 'plyr' does not even get loaded, hence added the extra 
      # ... library commmand that follows
    ...
    for (i in seq_along(MIT_value)) {
      ...
      # 'gap_times_function'
       ...
       # create empty df
          ...
          gap_times_function <- function(RainEvents_list_2) {
      ...
      # enter a loop to filter the rainfall events from the master dataframe - based on the Year - first try
      
        # create empty list
      
      RainEvents_list_2_by_year<-list(NULL)
      
      for (v in seq_along(year_as_factor)) {
        
        ## STOPPED HERE 22-FEB-2021
        
        print(v)
        
          # filter by the Year
            
          subsetted_events <- RainEvents_df_long %>% filter(Year==year_as_factor[v])
          
          print(head(subsetted_events))
          
          # create a list of the subsetted rainfall events
          
          RainEvents_list_2_by_year[v]<-list(subsetted_events)
      ...
      year_as_factor <- as.numeric(levels(year_as_factor))
     ...
     # use 'map' to apply 'gap_times_function' to the list of dataframes
      ...
      # gap_results <- TEST_RainEvents_split %>% map(gap_times_function)
