...
# note to self; load 'plyr' before 'dplyr' / 'tidyverse'
...
```{r compile-raw-data-PART-2, eval = FALSE, include=FALSE}
...
\blandscape
```{r stats-part-1, eval= TRUE, echo = FALSE,include = TRUE, message=FALSE, warning=FALSE, results = 'asis',fig.width=14,fig.height=10}
...
library(plyr)      
      ...
      # detach 'dplyr' package; due to conflicts with 'tidyverse' package
      detach("package:plyr")   
...
```{r charts-part-2, eval= TRUE,echo = FALSE,include = TRUE, message=FALSE, warning=FALSE, results = 'asis',fig.width=9,fig.asp=0.618,fig.align='centre'}
...
```{r charts-part-3,eval= TRUE,echo = FALSE,message=FALSE, warning=FALSE}
...
```{r duration_analysis-part-1, eval=TRUE, echo = FALSE,message=FALSE, warning=FALSE}
...
# the read function was removed becuase the 5-minute rainfall data was already read in a previous chunk
    ...
    for (i in seq_along(MIT_value)) {
      ...
      rm(Rain_Over_0,Rainindex)
      
      rm(RainEvents_list_1,RainEvents_list_2)
      ...
      # last_line_df<- RainEvents_df_long %>% group_by(.id) %>% summarize(depth=last(cumulative.rainfall), duration.minutes=last(event.duration.minutes),duration.hours=last(event.duration.hours),rainrate.mm.hr=last(event.rainrate.mm.hr),max.intensity.mm.hr=max(intensity.mm.hr,na.rm=TRUE))
       ...
       depth<- RainEvents_df_long %>% group_by(.id) %>% summarize(depth=last(cumulative.rainfall)) %>% select(depth) %>% ungroup()
       
       duration_hours<- RainEvents_df_long %>% group_by(.id) %>% summarize(duration=last(event.duration.hours)) %>% select(duration) %>% ungroup()
       
       intensity<- RainEvents_df_long %>% group_by(.id) %>% summarize(intensity=last(event.rainrate.mm.hr)) %>% select(intensity) %>% ungroup()
      ...
      CSV_outfile<-file.path(stats_folder_3, CSV_file_name)
       ...
       ## revised approach to getting the gap times, which must be done by year
       
       # try to split 'RainEvents_df_long' by Year, using the Year as a factor
       
      year_as_factor <- factor(RainEvents_df_long$Year)
       
      # view(levels(year_as_factor))
      
      # create a list of the rainfall events, split by year
      
      RainEvents_df_long_split <- split(RainEvents_df_long, year_as_factor)
       
      
      ## determine the gap time between rainfall events
      
      # apply 'gap_times_function to the list 'RainEvents_df_long_split
      
      
      gap_times_function <- function(x) {
          ...
          gap_times_df <- NULL
             ...
             for (m in seq_along(x)) {
                ...
                if (m > 1) {
                  ...
                  gap_start <- max(x[[m-1]]$Date.Time)
                  
                  gap_end <- min(x[[m]]$Date.Time)
      ...
      } # end of 'gap_times_function'
        
     
      
      
     # use 'map' to apply 'gap_times_function' to the list of dataframes

      # gap_results <- RainEvents_df_long_split %>% map(gap_times_function)
      
      
      # use 'bind_cols' to combine the list of dataframes to one dataframe

      # gap_results_bind_cols <- bind_cols(gap_results)

      # add column names
      
      # colnames(gap_results_bind_cols) <- c("Hourly","Daily","Weekly","Monthly","Yearly")
      ...
      CSV_file_name<-paste0(site_prefix,"_Statistics_Rainfall_Gap Times","_MIT_",MIT_value[i],".csv")
    ...
    # remove any previous CSV filenames
      
    # rm(CSV_file_name,CSV_outfile)
...
```{r testing-basic-stats, eval= FALSE, echo = FALSE,include = TRUE, message=FALSE, warning=FALSE, results = 'asis',fig.width=14,fig.height=10}
...
# testing area (19-Feb-2021); summarize on Chunk 3 not working
 ...
 # read a single .CSV file; generate basic summary stastics
    ...
    csv_name<-list.files('data_out/MASTER_CSV_5_min')
      ...
      rain_hourly <- rain_5_min_may_sept %>% group_by(Year,Month,Week,Day,Hour)  %>% summarize(hourlyRainfall = sum(Rainfall))
      
      rain_yearly <- rain_5_min_may_sept %>% group_by(Year)  %>% summarize(yearlyRainfall = sum(Rainfall))
